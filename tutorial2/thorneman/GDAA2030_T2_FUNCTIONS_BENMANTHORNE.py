...
def loadSceneList(sceneList, sceneTest=None, cloudCoverMax=None): 
    ...
    # using the pandas library, read the scene list into a dataframe object
    sceneDf = pd.read_csv(sceneList , nrows = sceneTest)

    # set the acquisition date field to a datetime object 
    ...
    # filter the dataset down to just clouds less than a given amount
    if cloudCoverMax: 
