...
# Purpose: acquire an image. imports to here
# A.Drake  Feb 11 2020
#=================================
...
def loadSceneList(sceneList, sceneTest=None, cloudCoverMax=None):
    #using pandas, read the scene list text file into a dataframe object
    ...
    if cloudCoverMax:
        #lets filter the dataset down to adjust clouds less than a given amount
        cloudFilter = sceneDf['cloudCover'] < cloudCoverMax
        sceneDf = sceneDf[cloudFilter]
    return sceneDf

def findPathRow(sceneDf, lat, lon):
    #calculate the distance from out latLon to each scene
...
def selectScene(sceneDf, path, row):
    ...
    #now find the newest capture in teh nearest path/row
